...
function Map() {
  ...
  const handleCheckIn = ({ latitude, longitude }: { latitude: number; longitude: number }) => {
    console.log("handleCheckIn in Map.tsx called with:", latitude, longitude);
    setLocation({ latitude, longitude });
  };
  ...
  return (
    <div className="relative h-screen w-full">
      <div className="absolute z-1 h-full w-full">
        <Mapbox
          {...viewState}
          onMove={(evt: { viewState: React.SetStateAction<{ longitude: number; latitude: number; zoom: number }>; }) =>
        ...
        >
          ...
          <LocationMarker onCheckIn={handleCheckIn} />
